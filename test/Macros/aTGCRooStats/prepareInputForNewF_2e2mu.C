void prepareInputForNewF_2e2mu(){
  ...
  TFile * file_signal=new TFile("/afs/cern.ch/work/s/senka/aTGC_ZZ_7TeV/fgZzzMix_eemm_f5_aTGC_final.root","read");
  file_signal->cd();
  TH1F * histo_SM=new TH1F("histo_SM","histo_SM",5,bins);
  eleEleMuMuEventTreeMerged->Draw("mass>>histo_SM","weightnoPU*(f5z_grid==0.&&f5g_grid==0.)");
  ...
  eleEleMuMuEventTreeMerged->Draw("mass>>histo_data","");
  ...
  TFile * file_out=new TFile("elmu_boosted.root","recreate");
  file_out->cd();
  histo_SM->Write("diboson");
  histo_data->Write("data_obs");
  histo_bkg->Write("background");
  histo_bkg->Scale(1.3);
  histo_bkg->Write("background_elmuboosted_backshapeUp");
  histo_bkg->Scale(0.7/1.3);
  histo_bkg->Write("background_elmuboosted_backshapeDown");
