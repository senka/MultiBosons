void prepareInputForNewF_4mu_8TeV(){
  ...
  TFile * file_signal=new TFile("/afs/cern.ch/work/s/senka/aTGC_ZZ_8TeV_corrected/f5_mmmm_aTGC_f5Zf5G_take2.root","read");
  file_signal->cd();
  TH1F * histo_SM=new TH1F("histo_SM","histo_SM",10,bins);
  mmmmFinalhighMass->Draw("mass>>histo_SM","weight*(f5z_grid==0.&&f5g_grid==0.)");
  ...
  mmmmFinalhighMass->Draw("mass>>histo_data","");
  ...
  TFile * file_out=new TFile("mu4_boosted_8TeV.root","recreate");
  file_out->cd();
  histo_SM->Write("diboson");
  histo_data->Write("data_obs");
  histo_bkg->Write("background");
  histo_bkg->Scale(1.3);
  histo_bkg->Write("background_muboosted_backshapeUp");
  histo_bkg->Scale(0.7/1.3);
  histo_bkg->Write("background_muboosted_backshapeDown");
