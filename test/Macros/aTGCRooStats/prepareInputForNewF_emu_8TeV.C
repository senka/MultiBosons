void prepareInputForNewF_emu_8TeV(){
  ...
  TFile * file_signal=new TFile("/afs/cern.ch/work/s/senka/aTGC_ZZ_8TeV_corrected/f5_mmee_aTGC_f5Zf5G_take2.root","read");
  file_signal->cd();
  TH1F * histo_SM=new TH1F("histo_SM","histo_SM",10,bins);
  mmeeFinalhighMass->Draw("mass>>histo_SM","weight*(f5z_grid==0.&&f5g_grid==0.)");
  ...
  mmeeFinalhighMass->Draw("mass>>histo_data","");
  ...
  TFile * file_out=new TFile("emu_boosted_8TeV.root","recreate");
  file_out->cd();
